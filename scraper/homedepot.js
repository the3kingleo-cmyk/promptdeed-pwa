...
const STORES = [
  // Add your local store IDs here — find yours at homedepot.com/l/store-finder
  // Defaults to nationwide clearance
];
...
async function scrape() {
  ...
  try {
    ...
    const items = await page.evaluate(() => {
      ...
      // Fallback: structured JSON-LD data
      if (results.length === 0) {
...
(async () => {
  ...
  // Save/overwrite latest markdown
